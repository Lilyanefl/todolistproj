...
export class UsersvcService {
  ...
  getUniqueUsers(todosUtenti: iTodosUsers[]): iUser[] {
    const uniqueUserIds = Array.from(
      new Set(todosUtenti.map((item) => item.user.id))
    );
    return uniqueUserIds
      .map((id) => {
        const foundTask = todosUtenti.find((task) => task.user.id === id);
        if (foundTask) {
          return foundTask.user;
        }
        return null;
      })
